...
export function PayoutTable({ payout, refetch, adminApprove }) {
    ...
    () => [
      ...
      },
      {
        ...
        Cell: (row) => (
          <p
            onClick={() =>
              gotoDetailsPage(row.row.original.transaction.project.id)
            }
          >
            {row.value}
          </p>
      ...
      },
      {
        ...
        Cell: (props) =>
          props.value ? (
            <a href={props.value} target="_blank" className="text-blue-700 fs-400 text-center rounded-lg border border-blue-700 block py-1">
              View Receipt
            </a>
          ) : (
