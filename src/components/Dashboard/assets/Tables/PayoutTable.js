...
export function PayoutTable({ payout, adminApprove }) {
  ...
  const handleUpload = (e) => {
    ...
    axios.post('https://api.buildonthego.com/upload', fd, {
      headers: {
      ...
      .then((res) => {
        ...
        axios.post(`https://api.buildonthego.com/api/transaction/save-proof/${selected}`, payload, {
          header: {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        Cell: (props) => cutText(props.value, 38)
      },
      {
        Header: "Total Amount",
      ...
      },
      {
        ...
        accessor: "trx_proof",
        Cell: (props) => props.value ? <a href={props.value} target='_blank'>View Receipt</a> : 'Null'
      },
      ...(admin?.level === 3? [{
        Header: 'Action',
        accessor: 'id',
        Cell: (row) => <Menu placement="left-start" className="w-16">
          ...
          <MenuList className="w-16 bg-gray-100 fw-600 text-black">
            <MenuItem className='bg-green-600 hover:bg-red-500 text-white text-center' onClick={() => openUpload(row.value)}>Upload Receipt</MenuItem>
          </MenuList>
        </Menu>,
      }] : [{
        Header: '',
    ...
    <>
      ...
      {uploadModal && (
        ...
        >
          ...
          >
            <p className='mb-6 text-center fw-600 text-xl'>Add Proof of Payment</p>
            <form onSubmit={handleUpload}>
              <div>
                <p>Receipt File</p>
                <input type="file" className='w-full p-2 rounded border border-gray-400 mt-2' onChange={(e) => setUploadFile(e.target.files[0])} />
              </div>
