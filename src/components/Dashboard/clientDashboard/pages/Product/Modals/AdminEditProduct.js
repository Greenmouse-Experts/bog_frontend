...
const AdminEditProduct = ({ CloseModal, product }) => {
  ...
  const { name, price, quantity, description, max_qty, min_qty } =
    formik.values;
  ...
  const index = options.findIndex(
    (where) => where.value === product.categoryId
  );
      ...
      >
        <form onSubmit={formik.handleSubmit}>
          ...
          <div className="mt-8 flex  justify-between">
            <Button color="red" className="lg:px-12" onClick={CloseModal}>
              Cancel
            </Button>
            {loading ? (
              <Spinner />
            ) : (
              <Button type="submit" className="bg-primary lg:px-12">
                Edit Product
              </Button>
            )}
          </div>
