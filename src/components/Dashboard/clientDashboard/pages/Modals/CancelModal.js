...
const CancelModal = ({ CloseDelete, meetingId, setFeetback, updateMeetingStatus }) => {
        ...
        <div className="fixed font-primary left-0 top-0 w-full h-screen bg-op center-item z-40" onClick={CloseDelete}>
            <div className="bg-white lg:w-5/12 rounded-md  overscroll-none  w-11/12 pt-8 shadow fw-500 scale-ani" onClick={e => e.stopPropagation()}>
                <div className="flex lg:px-6 px-5">
                    ...
                    <div>
                        <p className="fs-700 fw-600 mb-4">Cancel Meeting</p>
                        <p>Are you sure you want to cancel this meeting request? This action cannot be undo</p>
                    </div>
                </div>
                <div className="bg-light rounded-b-md  py-4 mt-5 text-end px-5">
                    <Button color="black" variant="outlined" ripple={true} onClick={CloseDelete}>Back</Button>
                    {
                        loading ? <Spinner /> :
                            <Button color="red" onClick={declineMeeting} className="ml-4" ripple={true}>Cancel Request</Button>
                    }
